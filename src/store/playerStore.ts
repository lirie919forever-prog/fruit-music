...
function isPlaylist(value: unknown): value is Playlist {
  ...
  return typeof playlist.id === 'string' && playlist.id !== ''
...
export function sanitizePersistedState(value: unknown): Partial<PlayerState> {
  ...
  const playlists = Array.isArray(raw.playlists)
    ? raw.playlists.filter(isPlaylist).map((playlist) => ({ ...playlist, songs: dedupeById((playlist.songs as unknown[]).filter(isSong)) }))
    : undefined;
...
export function createPlayerStore(initialView: ViewType = 'albums', initialQuery = '') {
  ...
  addToQueue: (song) => set((state) => ({ queue: [...state.queue, { song, addedBy: 'user' }], shuffleOrder: [] })),
  ...
  toggleShuffle: () => set((state) => {
    ...
    return {
      shuffle,
      shuffleOrder: shuffle && state.queueIndex !== null ? buildShuffleOrder(state.queue.length, state.queueIndex) : [],
    };
  }),
