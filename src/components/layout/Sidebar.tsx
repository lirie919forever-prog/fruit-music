...
function IconSparkles() {
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5">
      <path d="m8 1 1.2 4.8L14 7l-4.8 1.2L8 13l-1.2-4.8L2 7l4.8-1.2L8 1Z" />
      <path d="m13 11 .4 1.6L15 13l-1.6.4L13 15l-.4-1.6L11 13l1.6-.4L13 11Z" />
    </svg>
  );
}
...
function NavSections({ onSelect, onNavigate }: { onSelect?: () => void; onNavigate?: (view: ViewType) => void }) {
  ...
  return navigationSections.map((section) => (
    <div key={section.title}>
      <p className="px-2 pb-1 text-[11px] font-semibold uppercase tracking-[0.06em] text-[var(--salt-mist)]">
        {section.title}
      </p>
  ...
  return (
    <aside className="hidden h-dvh w-[248px] shrink-0 border-r border-[var(--glass-border)] bg-[#f3f8fb] md:flex md:flex-col">
      <div className="px-4 pb-4 pt-6">
        <div className="flex items-center gap-2.5 px-2 text-[var(--salt-white)]">
          <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-white text-[var(--salt-primary)] shadow-[0_1px_3px_rgba(48,145,198,0.18)]">
            <WaveIcon />
          </span>
          <span className="text-[19px] font-bold tracking-[-0.02em]">Marea</span>
        </div>
      </div>
      <nav className="flex flex-1 flex-col gap-4 overflow-y-auto px-2 pb-6">
          ...
          <div className="absolute inset-y-0 left-0 w-[min(86vw,320px)] overflow-y-auto border-r border-[var(--glass-border)] bg-[#f3f8fb] p-2 shadow-[18px_0_60px_rgba(25,74,102,0.2)]">
            <div className="flex items-center justify-between px-2 pb-3 pt-2">
              <div className="flex items-center gap-2.5 text-[19px] font-bold text-[var(--salt-white)]">
                <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-white text-[var(--salt-primary)]">
                  <WaveIcon />
                </span>
                Marea
              </div>
              <button
                ...
                onClick={closeNavigation}
                aria-label="Close navigation"
                className="flex h-9 w-9 items-center justify-center rounded-full text-[var(--salt-mist)] hover:bg-[var(--glass-bg-hover)]"
              >
