    ...
    <article className={`min-w-0 ${centered ? 'text-center' : ''}`}>
      ...
      >
        <span className={`relative block aspect-square overflow-hidden bg-[var(--salt-ghost)] ${rounded}`}>
          <CoverArt src={coverArt} alt="" loading="lazy" decoding="async" className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-[1.03] group-focus-visible:scale-[1.03]" />
      ...
      {errorText && (
        <p className="mt-1 text-xs text-[var(--danger)]">
          {errorText}{' '}
          <button type="button" onClick={retry} className="rounded underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--salt-primary)]">Try again</button>
        </p>
      )}
    </article>
  );
...
export function ArtistTile({ artist, onNavigateWithItem }: { artist: Artist } & TileNavProps) {
  ...
  return (
    <TileShell
      ...
      centered
    >
      <span className="mt-2 block truncate text-center text-[13px] font-medium text-[var(--salt-white)]">{artist.name}</span>
    </TileShell>
  );
}

/** The grid every tile row uses, so column counts never diverge between views. */
export const TILE_GRID = 'grid grid-cols-2 gap-x-4 gap-y-6 min-[420px]:grid-cols-3 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6';
    ...
    <div className={TILE_GRID}>
      {Array.from({ length: count }).map((_, index) => (
        <div key={index} className="space-y-2">
          <div className={`aspect-square animate-pulse bg-[var(--salt-ghost)] ${circular ? 'rounded-full' : 'rounded-md'}`} />
