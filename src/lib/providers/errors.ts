...
function composeSignals(external: AbortSignal | undefined, timeout: AbortSignal): { signal: AbortSignal; cleanup: () => void } {
...
export async function providerFetch<T>(
  ...
  const timeout = setTimeout(() => timeoutController.abort(new DOMException('Timed out', 'TimeoutError')), PROVIDER_TIMEOUT_MS);
